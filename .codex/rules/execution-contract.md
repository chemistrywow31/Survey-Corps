...
- **EC-2.3** Global cap: one initial attempt plus at most two retries = three attempts per subtask, across both tiers. This is the same 3-attempt bound as `.codex/rules/anti-sycophancy.md` — do not restate it here. One exception: when the cap is consumed and opus has not yet attempted, opus receives exactly one escalation attempt with the full trace.
...
- **EC-3.1** No self-verification: the agent that produced an artifact must not perform its acceptance, and Levi (who dispatched it) must never be the sole acceptor. Producer Self-Critique per `.codex/rules/reasoning-and-self-critique.md` is mandatory but never counts as acceptance.
- **EC-3.2** Acceptance is performed by a fresh-context verifier Levi launches via `spawn_agent` (driven with `send_input` / `wait_agent`). If delegation is unavailable, Levi reads the artifact in a fresh pass against the criteria only and records the fallback in the worklog. The verifier receives only the acceptance criteria and the artifact paths — never the producer's reasoning, drafts, or history. Reuse a non-producing agent (e.g. the other investigator checks a dossier; a fresh pass checks the merged Source Registry arithmetic). Petra remains the final-report QA.
...
1. Safety: `.codex/config.toml` and Codex sandbox guards, plus destructive-action guards
2. Charter: `AGENTS.md`, `CLAUDE.md`, and this contract
...
Tradeoff: EC-3 adds one verifier pass per accepted deliverable. This buys detection of false "done" claims before they propagate downstream — in the advanced-tech-advisory run, corrupted merge scores reached Phase 5 precisely because no independent verification existed.
