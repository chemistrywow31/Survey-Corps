...
- Applies to: `quality-inspector`, `report-producer`, `commander`
...
5. **Technical Feasibility** — Tech Assessor's evaluation of implementation viability, technology stack recommendations, risk factors.
...
The Report Producer must generate the report in all requested output formats. The HTML version serves as the master document; all other formats must contain identical content.
...
The Quality Inspector must verify every item before approving the report:
...
- [ ] Debate Analysis accurately reflects both Pro and Con positions.
...
When the Quality Inspector identifies issues:
1. Produce a QA Report listing each issue with severity (critical / high / medium / low).
2. Send the QA Report to the Commander.
3. The Commander routes each issue to the responsible agent for revision.
4. After revisions, the Quality Inspector re-inspects issues marked critical or high.
...
- Quality Inspector approves a report without completing the full checklist → Violation
- Report Producer delivers HTML and PPTX with different content (e.g., missing section in PPTX) → Violation
- Executive Summary exceeds 400 words → Violation
- Quality Inspector identifies a critical issue but does not require re-inspection after revision → Violation
